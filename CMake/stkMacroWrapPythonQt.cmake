...
#!
#!    TARGET ...........: Name of the wrapped library. For example: STKWidget
#!
...
#!
#! LOG FILE:
#!   File stkMacroWrapPythonQt_log.txt will be created in the current directory.
...
function(stkMacroWrapPythonQt_log msg)
  if(verbose)
    message(${msg})
  endif()
  file(APPEND "${CMAKE_CURRENT_BINARY_DIR}/stkMacroWrapPythonQt_log.txt" "${msg}\n")
endfunction()

include(${STK_CMAKE_DIR}/stkMacroSetPaths.cmake)
...
#!
#! \ingroup CMakeUtilities
function(stkMacroWrapPythonQt_reSearchFile python_exe python_library_path regex file is_matching)
...
")
  ...
  stkMacroSetPaths("${python_library_path}")
  execute_process(
...
#! \ingroup CMakeUtilities
macro(stkMacroWrapPythonQt WRAPPING_NAMESPACE TARGET SRCS_LIST_NAME SOURCES IS_WRAP_FULL HAS_DECORATOR)

  # Sanity check
  if(IS_WRAP_FULL AND NOT EXISTS "${PYTHONQTGENERATOR_EXECUTABLE}")
    message(FATAL_ERROR "PYTHONQTGENERATOR_EXECUTABLE not specified or inexistent when calling stkMacroWrapPythonQt")
  endif()

  # TODO: this find package seems not to work when called form a superbuild, but the call is needed
  # in general to find the python interpreter.  In STK, the toplevel CMakeLists.txt does the find
  # package so this is a no-op.  Other uses of this file may need to have this call so it is still enabled.
  find_package(PythonInterp)
  if(NOT PYTHONINTERP_FOUND)
    message(FATAL_ERROR "PYTHON_EXECUTABLE not specified or inexistent when calling stkMacroWrapPythonQt")
  endif()
  ...
  # Clear log file
  file(WRITE "${CMAKE_CURRENT_BINARY_DIR}/stkMacroWrapPythonQt_log.txt" "")
  ...
  foreach(FILE ${SOURCES})
    ...
    if(NOT skip_wrapping)
      ...
      if(NOT ${FILE} MATCHES "^.*\\.[hH]$")
        ...
        stkMacroWrapPythonQt_log("${FILE}: skipping - Not a regular header")
      endif()
    endif()
    ...
    if(NOT skip_wrapping)
      ...
      if(${FILE} MATCHES "^.*_[pP]\\.[hH]$")
        set(skip_wrapping TRUE)
        stkMacroWrapPythonQt_log("${FILE}: skipping - Pimpl header (*._p.h)")
      endif()
    endif()
    ...
    if(NOT skip_wrapping)
      ...
      if(skip_wrapping)
        stkMacroWrapPythonQt_log("${FILE}: skipping - WRAP_EXCLUDE")
      endif()
    endif()
    ...
    if(NOT skip_wrapping)
      ...
      if(NOT IS_WRAP_FULL)
        ...
        if(NOT "${file_content}" MATCHES "Q_OBJECT")
          set(skip_wrapping TRUE)
          stkMacroWrapPythonQt_log("${FILE}: skipping - No Q_OBJECT macro")
        endif()
      endif()
    endif()
    ...
    if(NOT skip_wrapping)
      ...
      if(NOT IS_WRAP_FULL)
        ...
        stkMacroWrapPythonQt_reSearchfile(${PYTHON_EXECUTABLE} ${PYTHON_LIBRARY_PATH}
                                          ${regex} ${CMAKE_CURRENT_SOURCE_DIR}/${FILE} is_matching)
        if(NOT is_matching)
          set(skip_wrapping TRUE)
          stkMacroWrapPythonQt_log("${FILE}: skipping - Missing expected constructor signature")
        endif()
      endif()
    endif()
    ...
    if(NOT skip_wrapping)
      ...
      if(is_matching)
        set(skip_wrapping TRUE)
        stkMacroWrapPythonQt_log("${FILE}: skipping - Contains a virtual pure method")
      endif()
    endif()
  ...
  foreach(include ${STK_BASE_INCLUDE_DIRS})
  ...
  if(${IS_WRAP_FULL})
    set(wrap_type "Full")
    set(extra_files ${wrap_int_dir}stkPythonQt_${TARGET}_masterinclude.h)
  endif()
  ...
  add_custom_command(
    ...
    DEPENDS
      ...
      ${SOURCES_TO_WRAP}
      ${STK_CMAKE_DIR}/stkScriptWrapPythonQt_${wrap_type}.cmake
      ${STK_CMAKE_DIR}/stkMacroWrapPythonQtModuleInit.cpp.in
    COMMAND ${CMAKE_COMMAND}
      ...
      -P ${STK_CMAKE_DIR}/stkScriptWrapPythonQt_${wrap_type}.cmake
  ...
  add_custom_command(
    ...
    DEPENDS
      ${wrap_int_dir}${wrapper_init_cpp_filename}
      ${wrap_int_dir}${wrapper_module_init_cpp_filename}
      ${extra_files} ${STK_CMAKE_DIR}/stkScriptMocPythonQtWrapper.cmake
    COMMAND ${CMAKE_COMMAND}
      -DWRAPPING_NAMESPACE:STRING=${WRAPPING_NAMESPACE}
      -DTARGET:STRING=${TARGET}
      ...
      -P ${STK_CMAKE_DIR}/stkScriptMocPythonQtWrapper.cmake
